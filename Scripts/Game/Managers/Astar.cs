...
namespace Twelve.Game.Mangers
{
    public class AStar
    {
        ...
        private void ResolveRoute(Vector2Int startCoordinates, Vector2Int goalCoordinates, List<Vector2Int> result)
        {
        ...
        Vector2Int GetBestScoreCoordinates()
        {
            ...
            foreach (var v in openCoordinates.Where(v => min > nodes[v.x, v.y].GetScore()))
            {
                // 優秀なコストの更新(値が低いほど優秀)
